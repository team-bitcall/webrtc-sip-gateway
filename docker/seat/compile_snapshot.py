...
def render_snapshot(normalized):
    """Render normalized data; all untrusted string values are hex-decoded at runtime."""
    lines = [
        "#!KAMAILIO",
        "# Generated by compile_snapshot.py; do not edit.",
        "event_route[htable:mod-init] {",
        "  " + _set_string("seat_meta", "domain", normalized["domain"]),
        "  " + _set_int("seat_meta", "revision", normalized["revision"]),
        "  " + _set_int("seat_meta", "valid_until", normalized["validUntil"]),
    ]
    for seat in normalized["seats"]:
        prefix = seat["username"] + "::"
        for field, value in (
            ("id", seat["id"]),
            ...
            lines.append("  " + _set_string("seat_users", prefix + field, value))
        lines.append("  " + _set_int("seat_users", prefix + "enabled", int(seat["enabled"])))
    for profile in normalized["profiles"]:
        ...
        for field, value in fields:
            lines.append("  " + _set_string("seat_profiles", prefix + field, value))
        lines.append("  " + _set_int("seat_profiles", prefix + "enabled", int(profile["enabled"])))
    lines.append("}")
    return "\n".join(lines) + "\n"
